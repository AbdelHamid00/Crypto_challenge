...
func detectSingleCharXOR(file *os.File) (plaintext string, Key byte, err error) {
	scanner := bufio.NewScanner(file)
	minScore := float64(math.Inf(1))
	// Read the file line by line
	for scanner.Scan() {
		line := scanner.Text()
		plaintextBytes , err := hex.DecodeString(line)
		if err != nil {
...
func main() {
	file, err := os.Open("./InputFiles/input4.txt")
	if err != nil {
		fmt.Println("Error opening file:", err)
		return
	}
	defer file.Close()
	plaintext, Key , errr := detectSingleCharXOR(file)
	if errr != nil {
		panic(err)
	}
